...
def efficient_b3(cfg, pretrain=True, pretrained_model=None, last_layer_stride=1):
    model = EfficientNet(type_str='b3', pretrain_path=pretrained_model if pretrain else "")
    return model

def efficient_b4(cfg, pretrain=True, pretrained_model=None, last_layer_stride=1):
    model = EfficientNet(type_str='b4', pretrain_path=pretrained_model if pretrain else "")
    return model

def efficient_b5(cfg, pretrain=True, pretrained_model=None, last_layer_stride=1):
    model = EfficientNet(type_str='b5', pretrain_path=pretrained_model if pretrain else "")
    return model

def efficient_b6(cfg, pretrain=True, pretrained_model=None, last_layer_stride=1):
    model = EfficientNet(type_str='b6', pretrain_path=pretrained_model if pretrain else "")
    return model
...
class EfficientNet(nn.Module):
    ...
    -----------------------------
    Hyper-Parameter:
    type_str: the type of the EfficientNet to use, must be in [b0, b1, b2, b3, b4, b5, b6, b7]
    pretrain_path: the path to the pretrained model, if is "", a random init model will be return,
    oss path: s3://model-zoo/regular_models/efficientnet-b[0-7].pth 
    """

    def __init__(self, type_str='b3', pretrain_path='None'):
        self.type_str = type_str
        self.pretrain_path = pretrain_path
        super(EfficientNet, self).__init__()

        model_name = "efficientnet-{}".format(self.type_str)
        blocks_args, global_params = from_name(model_name)
        ...
        if self.pretrain_path != "":
            load_pretrained_weights(self, model_name, load_fc=False, path=self.pretrain_path)
    ...
    def output_info(self) -> List[dict]:
        """
        给出该backbone输出的feature map的信息。每张feature map的信息是一个字典，格式是：
        {
            "down_sample_rate": int, 降采样率
            "channel_n": int, 通道数
        }
        """
        channel_n = 1792
        if self.type_str == "b7":
            channel_n = 2560
        elif self.type_str == "b6":
            channel_n = 2304
        elif self.type_str == "b5":
            channel_n = 2048
        elif self.type_str == "b4":
            channel_n = 1792
        elif self.type_str == "b3":
            channel_n = 1536
        elif self.type_str == "b2":
            channel_n = 1408
        elif self.type_str == "b1":
            channel_n = 1280
        elif self.type_str == "b0":
            channel_n = 1280
        return [{"down_sample_rate": 32, "channel_n": channel_n}]
...
def from_name(model_name, override_params=None):
    _check_model_name_is_valid(model_name)
...
def load_pretrained_weights(model, model_name, load_fc=True, path="", advprop=False):
