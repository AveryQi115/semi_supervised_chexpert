...
class SwitchWhiten2d(nn.Module):
...
class Bottleneck_SW(nn.Module):
    ...
    def __init__(self,
                 ...
                 sw_cfg=None):
        super(Bottleneck_SW, self).__init__()
...
class BBN_ResNet_SW(nn.Module):
    ...
    def __init__(self,
                 ...
                 stage_with_sw=(True, True, True, False)):
        self.inplanes = 64
        super(BBN_ResNet_SW, self).__init__()
        ...
        self.cb_block = block(self.inplanes, 512, norm_cfg=self.norm_cfg, sw_cfg=None)
        self.rb_block = block(self.inplanes, 512, norm_cfg=self.norm_cfg, sw_cfg=None)
    ...
    def _make_layer(self, block, planes, blocks, stride, with_sw):
        ...
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, norm_cfg=self.norm_cfg, sw_cfg=self.sw_cfg if(with_sw and i % 2 == 1) else None))
    ...
    def load_param(self, model_path):
        ...
        for i in param_dict:
            ...
            if 'module.' in i:
                if "layer4.2" in i:
                    self.state_dict()["cb_block" + i[15:]].copy_(param_dict[i])
                    self.state_dict()["rb_block" + i[15:]].copy_(param_dict[i])
                else:
                    self.state_dict()[i[7:]].copy_(param_dict[i])
            else:
                if "layer4.2" in i:
                    self.state_dict()["cb_block" + i[8:]].copy_(param_dict[i])
                    self.state_dict()["rb_block" + i[8:]].copy_(param_dict[i])
                else:
                    self.state_dict()[i].copy_(param_dict[i])
...
def res50_sw(
    ...
    resnet_sw = ResNet_SW(
        block=Bottleneck_SW,
        layers=[3, 4, 6, 3],
        sw_cfg=dict(type='SW', requires_grad=True)
    )
    if pretrain and pretrained_model != "":
        resnet_sw.load_param(model_path=pretrained_model)
        # resnet_sw.load_state_dict(torch.load(pretrained_model),strict=True)
    else:
...
def bbn_res50_sw(
    ...
    bbn_resnet_sw = BBN_ResNet_SW(
        block=Bottleneck_SW,
        layers=[3, 4, 6, 3],
        sw_cfg=dict(type='SW', requires_grad=True)
    )
    if pretrain and pretrained_model != "":
        bbn_resnet_sw.load_param(model_path=pretrained_model)
        # resnet_sw.load_state_dict(torch.load(pretrained_model),strict=True)
    else:
        print("Choose to train from scratch")
    return bbn_resnet_sw